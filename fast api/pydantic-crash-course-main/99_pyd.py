...
class Patient(BaseModel):
    ...
    name: str = Field(max_length = 50) # name length maximum under the 50 characters``
    email: EmailStr
    linkedin: AnyUrl
    age: int = Field(gt=0, lt=32) # custom data under 32 it range
    weight: float = Field(gt= 0) # apply custom constrct (show posive value)
    married: bool = False # defolt value
    allergies: Optional[List[str]] = Field(max_length =  5) # dont add in more 5 allergies
